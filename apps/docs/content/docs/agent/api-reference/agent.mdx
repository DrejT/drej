...
`@drej/agent` has no storage-adapter dependency of its own — every method below that takes `opts.adapter` requires it. Use `SQLiteAdapter` from `@drej/sqlite` for local dev or `PostgresAdapter` from `@drej/postgres` for production.
