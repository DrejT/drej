...
description: Write, read, move, delete, search, and list files inside the sandbox container.
---

`Sandbox` exposes six file operation methods. All operate on paths inside the container.
...
## Pattern: patch a file via exec

For in-place text replacements, use `exec()` with `sed`:
...
All file operations are available in `SandboxBuilder` for use in `@drej/workflow`. `readFile` takes an extra `as` argument to store the result in `vars`:
