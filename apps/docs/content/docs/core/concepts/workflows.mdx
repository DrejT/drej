...
await workflow(client)
  .sandbox({ image: "node:22" }, (sb) => { sb.exec("npm run build"); })
  .sandbox({ image: "ubuntu:22.04" }, (sb) => { sb.exec("./deploy.sh"); })
  .pipe(process.stdout);
...
await workflow(client)
  .sequence([
    { image: "node:22", name: "build", run: (sb) => sb.exec("npm run build") },
    { image: "ubuntu:22.04", name: "deploy", run: (sb, prev) => sb.exec("./deploy.sh") },
  ])
  .pipe(process.stdout);
```
