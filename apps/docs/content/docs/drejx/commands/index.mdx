...
These wrap `@drej/agent`'s `Agent.load()`/`Agent.resume()`/`Agent.attach()`/`Agent.spawn()` directly. Sessions are always addressed by **sandbox ID**, not name — see [drejx prompt](/docs/drejx/commands/prompt#why-sandbox-id-not-name) for why.
