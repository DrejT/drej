...
A drejx "registry item" is just a URL that returns an `AgentSpec` JSON object (the same spec type used by [`@drej/agent`](/docs/agent)). Any URL works — a GitHub raw file, a Gist, your own server, or the small curated set of examples drej hosts at [registry.drej.dev](https://registry.drej.dev) — there's no requirement to publish through a central service.
